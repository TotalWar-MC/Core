...
import com.steffbeard.totalwar.modules.npc.Config;
...
public abstract class CargoTask extends BukkitRunnable {
	...
	public void run() {
		...
		if (config.debugMode)
			NPCMain.logger.info("Running execute method for CargoTask with address " + this + ". Pilot: "
					+ originalPilot.getName() + " CraftSize: " + originalLocations.size() + " CraftType: "
					+ craft.getType().getCraftName() + " StockItem: "
