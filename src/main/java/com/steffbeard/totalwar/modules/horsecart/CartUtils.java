...
public class CartUtils {
	...
	public static BlockFace isCart(LivingEntity h){
		...
		if (leashHolder instanceof LeashHitch)
		{
			...
			{
				if ((axle.getRelative(CartUtils.relativeLeft(cartDirection)).getTypeId() == 96) && (axle.getRelative(CartUtils.relativeRight(cartDirection)).getTypeId() == 96))
				{
	...
	public static boolean cartMove(Block hitch, double xDirection, double zDirection, BlockFace cartDirection, LivingEntity h) {	
		...
		if (!airBlocks.contains(newAxle.getRelative(CartUtils.relativeRight(cartDirection)).getTypeId()))
		{
			...
			if ((newAxle.getRelative(CartUtils.relativeRight(cartDirection)).getLocation().getBlock().equals(h.getLeashHolder().getLocation().getBlock())) && (!alreadyChanged))
			{
		...
		if ((airBlocks.contains(belowHitch.getRelative(cartDirection.getOppositeFace()).getTypeId())) && (airBlocks.contains(belowHitch.getRelative(CartUtils.relativeLeft(cartDirection)).getTypeId())) && (airBlocks.contains(belowHitch.getRelative(CartUtils.relativeRight(cartDirection)).getTypeId())) && (airBlocks.contains(belowHitch.getTypeId())))
		{
