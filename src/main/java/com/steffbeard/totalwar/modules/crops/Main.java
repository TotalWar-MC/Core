...
public class Main extends JavaPlugin {
	...
	public void onEnable() {		
		...
		Main.LOG.info("Logging hack, log level is set to: " + Main.minLogLevel.toString());
		Main.LOG.info("Caching entire database? " + this.persistConfig.cacheEntireDatabase);
	...
	public Plant growAndPersistBlock(Block block, boolean naturalGrowEvent, GrowthConfig growthConfig, Block fruitBlockToIgnore, DropGrouper dropGrouper) {
		...
		Main.doLog(Level.FINER, "RealisticBiomes:growAndPersistBlock() called for block: " + block + " and is naturalGrowEvent? " + naturalGrowEvent);
		if (!persistConfig.enabled)
			return null;
		...
		if (!loadChunk && !plantManager.isChunkLoaded(chunckCoords)) {
			Main.doLog(Level.FINER, "Realisticbiomes.growAndPersistBlock(): returning 0.0 because loadChunk = false or plantManager.chunkLoaded(" + chunckCoords + " is false");
		...
		if (growthConfig == null) {
			Main.doLog(Level.FINER, "Realisticbiomes.growAndPersistBlock(): returning 0.0 because growthConfig = null");
		...
		if (plant == null) {
			Main.doLog(Level.FINER, "Realisticbiomes.growAndPersistBlock(): creating new plant and adding it");
		...
		if (plant.isFullyGrown()) {
			...
			Main.doLog(Level.FINER,"Realisticbiomes.growAndPersistBlock(): Removing fully grown plant");
			plantManager.removePlant(block);
		}
	...
	public void growPlant(Plant plant, Block block, GrowthConfig growthConfig, Block fruitBlockToIgnore, DropGrouper dropGrouper) {
			...
			if (fruitGrowthConfig.isPersistent()) {	
				if (!hasFruit && plant.getGrowth() >= 1.0) {	
					...
					if (freeBlock != null) {
						...
						Main.doLog(Level.FINER, "Realisticbiomes.growPlant(): fruit rate: " + fruitRate);
						
						plant.growFruit(updateTime, fruitRate);
					} else {
						Main.doLog(Level.FINER, "Realisticbiomes.growPlant(): no free block for fruit");
					}
		...
		if (growthPrevented) {
			Main.doLog(Level.FINER, "Realisticbiomes.growPlant(): growth prevented");
			plant.setGrowth(0.0);
		}
	}
