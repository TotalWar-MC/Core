...
import org.bukkit.craftbukkit.v1_12_R1.util.StructureGrowDelegate;
...
import com.steffbeard.totalwar.modules.crops.events.RealisticBiomesBlockBreakEvent;
import com.steffbeard.totalwar.modules.crops.events.RealisticBiomesBlockGrowEvent;
import com.steffbeard.totalwar.modules.crops.events.RealisticBiomesStructureGrowEvent;
...
public class BlockGrower {
	...
	public boolean growBlock(Block block, float growth, float fruitGrowth) {
		...
		if (stages == null) {
			CropsMain.doLog(Level.FINER, "BlockGrower.growBlock(): no stages for " + block.getType());
			return false;
		}
		
		CropsMain.doLog(Level.FINER, "BlockGrower.growBlock(): growing " + block.getType() + " growth: " 
		...
		state.setData(data);
		CropsMain.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesBlockGrowEvent(block, state));
	...
	public boolean generateTree(Block block, float growth, TreeType type) {
		CropsMain.doLog(Level.FINER, "BlockGrower.generateTree(): " + type);
		if (growth < 1.0f) {
			return false;
		}
		...
		try {
			...
			if (block.getWorld().generateTree(block.getLocation(), type, sgd)) {
				...
				// Call wrapper event.
				CropsMain.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesStructureGrowEvent(
				...
				CropsMain.doLog(Level.FINER, "generateTree reset data: " + states.size());
	...
	public boolean growColumn(Block block, float growth, DropGrouper dropGrouper) {
		...
		if (block.getRelative(BlockFace.DOWN).getType() == type) {
			...
			CropsMain.doLog(Level.WARNING, "BlockGrower.growColumn(): is not bottom block");
			return false;
		}
		...
		for (int i = 0; i < stage; i++) {
			...
			CropsMain.doLog(Level.FINE, "BlockGrower.growColumn(): stage " + i);
			if (block.getType() == type) {
				CropsMain.doLog(Level.FINE, "BlockGrower.growColumn(): block above is already " + type);
				// continue to looke for a free spot above
				continue;
			} else if (block.getType() != Material.AIR) {
				CropsMain.doLog(Level.FINE, "BlockGrower.growColumn(): block above blocking, prevented growth");
				...
				CropsMain.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesBlockGrowEvent(block, preChange));
				block.setType(type);
			}
		}
		return false;
	...
	private boolean instaBreakCactus(Block topBlock, DropGrouper dropGrouper) {
		for (Vector victor: surroundingBlocks) {
			...
			if (candidate.getType() != Material.AIR) {
				...
				CropsMain.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesBlockGrowEvent(topBlock, preChange));
				
				topBlock.setType(Material.CACTUS); // temporarily change for break event
				CropsMain.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesBlockBreakEvent(topBlock, null));
	...
	private void growFruit(Block block, float fruitGrowth) {
		...
		if (freeBlock != null) {
			...
			CropsMain.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesBlockGrowEvent(freeBlock, preChange));
