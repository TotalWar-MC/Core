...
public class BlockGrower {
	...
	public boolean growBlock(Block block, float growth, float fruitGrowth) {
		...
		if (stages == null) {
			Main.doLog(Level.FINER, "BlockGrower.growBlock(): no stages for " + block.getType());
			return false;
		}
		
		Main.doLog(Level.FINER, "BlockGrower.growBlock(): growing " + block.getType() + " growth: " 
		...
		state.setData(data);
		Main.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesBlockGrowEvent(block, state));
	...
	public boolean generateTree(Block block, float growth, TreeType type) {
		Main.doLog(Level.FINER, "BlockGrower.generateTree(): " + type);
		if (growth < 1.0f) {
			return false;
		}
		...
		try {
			...
			if (block.getWorld().generateTree(block.getLocation(), type, sgd)) {
				...
				// Call wrapper event.
				Main.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesStructureGrowEvent(
				...
				Main.doLog(Level.FINER, "generateTree reset data: " + states.size());
	...
	public boolean growColumn(Block block, float growth, DropGrouper dropGrouper) {
		...
		if (block.getRelative(BlockFace.DOWN).getType() == type) {
			...
			Main.doLog(Level.WARNING, "BlockGrower.growColumn(): is not bottom block");
			return false;
		}
		...
		for (int i = 0; i < stage; i++) {
			...
			Main.doLog(Level.FINE, "BlockGrower.growColumn(): stage " + i);
			if (block.getType() == type) {
				Main.doLog(Level.FINE, "BlockGrower.growColumn(): block above is already " + type);
				// continue to looke for a free spot above
				continue;
			} else if (block.getType() != Material.AIR) {
				Main.doLog(Level.FINE, "BlockGrower.growColumn(): block above blocking, prevented growth");
				...
				Main.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesBlockGrowEvent(block, preChange));
				block.setType(type);
			}
		}
		return false;
	...
	private boolean instaBreakCactus(Block topBlock, DropGrouper dropGrouper) {
		for (Vector victor: surroundingBlocks) {
			...
			if (candidate.getType() != Material.AIR) {
				...
				Main.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesBlockGrowEvent(topBlock, preChange));
				
				topBlock.setType(Material.CACTUS); // temporarily change for break event
				Main.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesBlockBreakEvent(topBlock, null));
	...
	private void growFruit(Block block, float fruitGrowth) {
		...
		if (freeBlock != null) {
			...
			Main.plugin.getServer().getPluginManager().callEvent(new RealisticBiomesBlockGrowEvent(freeBlock, preChange));
