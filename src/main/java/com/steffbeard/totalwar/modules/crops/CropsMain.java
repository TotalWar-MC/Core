...
import com.steffbeard.totalwar.modules.crops.listener.GrowListener;
import com.steffbeard.totalwar.modules.crops.listener.PlayerListener;
import com.steffbeard.totalwar.modules.crops.listener.SpawnListener;
import com.steffbeard.totalwar.modules.crops.persist.BlockGrower;
import com.steffbeard.totalwar.modules.crops.persist.ChunkCoords;
...
public class CropsMain extends Module {
	...
	public void onEnable() {		
		CropsMain.plugin = this;
		
		CropsMain.LOG = this.getLogger();
		...
		CropsMain.LOG.info("Logging hack, log level is set to: " + CropsMain.minLogLevel.toString());
		CropsMain.LOG.info("Caching entire database? " + this.persistConfig.cacheEntireDatabase);
	...
	public static void doLog(Level level, String message) {
		...
		if (CropsMain.LOG != null) {
			...
			if (level.intValue() >= CropsMain.minLogLevel.intValue() ) {
				CropsMain.LOG.info("[" + level.toString() + "] " + message);
	...
	public static void doLog(Level level, String message, Throwable wrong) {
		...
		if (CropsMain.LOG != null) {
			...
			// However if its ALL, then its set to Integer.MIN_VALUE, so everything will get logged. etc etc
			if (level.intValue() >= CropsMain.minLogLevel.intValue() ) {
				CropsMain.LOG.log(level, "[" + level.toString() + "] " + message, wrong);
	...
	public Plant growAndPersistBlock(Block block, boolean naturalGrowEvent, GrowthConfig growthConfig, Block fruitBlockToIgnore, DropGrouper dropGrouper) {
		...
		CropsMain.doLog(Level.FINER, "RealisticBiomes:growAndPersistBlock() called for block: " + block + " and is naturalGrowEvent? " + naturalGrowEvent);
		if (!persistConfig.enabled)
			return null;
		...
		if (!loadChunk && !plantManager.isChunkLoaded(chunckCoords)) {
			CropsMain.doLog(Level.FINER, "Realisticbiomes.growAndPersistBlock(): returning 0.0 because loadChunk = false or plantManager.chunkLoaded(" + chunckCoords + " is false");
		...
		if (growthConfig == null) {
			CropsMain.doLog(Level.FINER, "Realisticbiomes.growAndPersistBlock(): returning 0.0 because growthConfig = null");
		...
		if (plant == null) {
			CropsMain.doLog(Level.FINER, "Realisticbiomes.growAndPersistBlock(): creating new plant and adding it");
		...
		if (plant.isFullyGrown()) {
			...
			CropsMain.doLog(Level.FINER,"Realisticbiomes.growAndPersistBlock(): Removing fully grown plant");
			plantManager.removePlant(block);
		}
	...
	public void growPlant(Plant plant, Block block, GrowthConfig growthConfig, Block fruitBlockToIgnore, DropGrouper dropGrouper) {
			...
			if (fruitGrowthConfig.isPersistent()) {	
				if (!hasFruit && plant.getGrowth() >= 1.0) {	
					...
					if (freeBlock != null) {
						...
						CropsMain.doLog(Level.FINER, "Realisticbiomes.growPlant(): fruit rate: " + fruitRate);
						
						plant.growFruit(updateTime, fruitRate);
					} else {
						CropsMain.doLog(Level.FINER, "Realisticbiomes.growPlant(): no free block for fruit");
					}
		...
		if (growthPrevented) {
			CropsMain.doLog(Level.FINER, "Realisticbiomes.growPlant(): growth prevented");
			plant.setGrowth(0.0);
		}
	}
