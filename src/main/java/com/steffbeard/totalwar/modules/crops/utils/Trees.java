package com.steffbeard.totalwar.modules.crops.utils;
...
public class Trees {
	...
	public static TreeType getTreeType(Block block, GrowthMap growthMap) {
		...
		if (type == TreeType.JUNGLE) {
			if (!canGrowLArge(block, type)) {
				...
				CropsMain.doLog(Level.FINER, "getTreeType jungle tall");
			}

		} else if (type == TreeType.REDWOOD) {
			if (canGrowLArge(block, type)) {
				CropsMain.doLog(Level.FINER, "getTreeType mega redwood");
			...
			if (!canGrowLArge(block, type)) {
				CropsMain.doLog(Level.FINER, "getTreeType darkoak not 2x2");
				return null;
			}
			...
			if (block.getBiome() == Biome.SWAMPLAND || block.getBiome() == Biome.MUTATED_SWAMPLAND) {
				...
				type = TreeType.SWAMP;
				CropsMain.doLog(Level.FINER, "getTreeType swamp");
			}

		}
		
		CropsMain.doLog(Level.FINER, "Trees.getTreeType(): " + type);
	...
	public static TreeType getAlternativeTree(TreeType treeType, Block block, GrowthMap growthMap) {
		...
		CropsMain.doLog(Level.FINER, "Trees.getChance() is " + rate + " for " + altType + " at " + block.getLocation());
