...
public class Database {
	...
	public Database(PersistConfig baseconfig) {
		...
		if (baseconfig.user != null && baseconfig.host != null && 
			...
			CropsMain.doLog(Level.FINER, "creating chunk table (if necessary) with prepared statement:" + Database.makeTableChunk);
			...
			try (Connection connection = getConnection();
				...
				CropsMain.doLog(Level.SEVERE, "Unable to initialize chunk table in Database!", se);
			...
			try (Connection connection = getConnection();
				...
				CropsMain.doLog(Level.SEVERE, "Unable to initialize plant table in Database!", se);
				this.datasource = null;
				return;
			}
			...
			try (Connection connection = getConnection();
				...
				CropsMain.LOG.info("Could not update table - ignore if already updated. Error code: " + e.getErrorCode() + ", error message: " + e.getMessage());
			}

		} else {
			this.datasource = null;
			CropsMain.doLog(Level.SEVERE, "Database not configured and is unavaiable");
		}
	}
