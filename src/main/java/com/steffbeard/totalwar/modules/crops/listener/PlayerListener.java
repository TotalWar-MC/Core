package com.steffbeard.totalwar.modules.crops.listener;
...
public class PlayerListener implements Listener {
	...
	public PlayerListener(CropsMain plugin, GrowthMap growthConfigs) {
		this.plugin = plugin;
	...
	public void onPlayerInteractEvent(PlayerInteractEvent event) {
		...
		if (event.getAction() == Action.LEFT_CLICK_BLOCK) {
			...
			if (growthConfig == null) {
				CropsMain.doLog(Level.FINER, "No config found for \"" + event.getItem() + "\" : " + growthConfigs.keySet());
				return;
			}
			
			CropsMain.doLog(Level.FINER, "LEFT CLICK: " + event.getItem() + ", " + growthConfig);
		...
		if (plugin.persistConfig.enabled && growthConfig.isPersistent()) {
			...
			CropsMain.doLog(Level.FINER, "PlayerListener.onPlayerInteractEvent(): rate for " + materialName + " at block " + block + " is " + rate);
			rate = (1.0/(rate*(60.0*60.0/*seconds per hour*/)));
			CropsMain.doLog(Level.FINER, "PlayerListener.onPlayerInteractEvent(): rate adjusted to "  + rate);
			...
			if (plant == null) {
					...
					if (!Fruits.hasFruit(block)) {
						...
						if (growthConfig.isPersistent()) {
							...
							if (block != null) {
								...
								CropsMain.doLog(Level.FINER, "PlayerListener.onPlayerInteractEvent(): fruit rate for block " + block + " is " + fruitRate);
								fruitRate = (1.0/(fruitRate*(60.0*60.0/*seconds per hour*/)));
								CropsMain.doLog(Level.FINER, "PlayerListener.onPlayerInteractEvent(): fruit rate adjusted to "  + fruitRate);
				...
				CropsMain.doLog(Level.FINER, "PlayerListener.onPlayerInteractEvent(): plant fruit is: " + plant.getFruitGrowth());
				String amount = new DecimalFormat("#0.00").format(rate);
				event.getPlayer().sendMessage("§7[Realistic Biomes] \"" + materialName + "\": "+amount+" hours to maturity");

			} else {
				
				CropsMain.doLog(Level.FINER, "PlayerListener.onPlayerInteractEvent(): plant growth is: " + plant.getGrowth());
				String amount = new DecimalFormat("#0.00").format(rate);
